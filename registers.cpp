...
int* Registers::address(char *operand, Memory &memory, const Labels &labels)
{
  ...
  if(*operand == '.' || *operand == '_' || isalpha(*operand))  // label
  {
    value = labels.find(operand); return &value;
...
int Registers::get(const Registers::RegName regName) const
{
...
void Registers::set(const Registers::RegName regName, int value)
{
  if(regName >= eax && regName <= ecx)
    regs[regName] = value;
} // set()
